        ...
        <div className="bg-white/80 backdrop-blur-2xl px-10 py-12 rounded-3xl shadow-2xl border border-white/50 text-center">
          ...
          {step === 'success' && (
            <div className="animate-fade-in">
              ...
              <h2 className="text-2xl font-black text-slate-800 mb-3">Password Direset!</h2>
              <p className="text-sm font-medium text-slate-500 mb-8 px-4">
                Passwordmu berhasil diperbarui. Kamu sekarang dapat login dengan kredensial baru.
              </p>
              <button
          ...
          {step === 'invalid' && (
            <div className="animate-fade-in">
              ...
              <h2 className="text-2xl font-black text-slate-800 mb-3">Link Tidak Valid atau Kedaluwarsa</h2>
              <p className="text-sm font-medium text-slate-500 mb-8 px-4">
                Link reset password ini sudah kedaluwarsa atau tidak valid. Link hanya berlaku selama <strong className="text-slate-700">60 menit</strong>.
              </p>
              <button
          ...
          {step === 'form' && (
            <div className="animate-fade-in">
              ...
              <h2 className="text-2xl font-black text-slate-800 mb-2">Password Baru</h2>
              <p className="text-sm font-medium text-slate-500 mb-8 px-4">
                Buat password baru yang kuat untuk <strong className="text-slate-700">{email}</strong>
              </p>

              <form onSubmit={handleReset} className="space-y-5 text-left">
                {/* New Password */}
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-widest mb-2 px-1">Password Baru</label>
                ...
                {/* Confirm Password */}
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-widest mb-2 px-1">Konfirmasi Password</label>
                  <div className="relative group">
                    <FiLock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 transition-colors group-focus-within:text-[#21569A]" />
                    <input
                  ...
                  {confirmPassword && confirmPassword !== password && (
                    <p className="text-xs text-red-400 font-semibold mt-1.5 px-1">Password tidak cocok</p>
                  )}
                </div>
                ...
                >
                  {isLoading ? <span className="animate-pulse">Menyimpan...</span> : "Reset Password"}
                </button>
              </form>
            </div>
          )}
